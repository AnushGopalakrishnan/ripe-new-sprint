...
- Editor nested-preview fix on 2026-06-18: selecting/reapplying a drafted ancestor such as `MAIN` could reset nested child previews because `src/lib/editor/bridge-source.ts` restored `element.innerHTML` for non-image previews. The bridge now stores/restores only the original inline style and optional text/image values, never whole subtree HTML for style-only parent previews. Verified with a focused bridge repro on local `/__editor?path=%2Fcareers`: apply a `MAIN` preview, apply nested `SECTION` `padding: 0px`, reapply `MAIN`; the section keeps inline `padding: 0px !important` and computed padding remains `0px`. `pnpm typecheck`, `pnpm lint`, and `git diff --check` passed.
- Merge cleanup on 2026-06-18: latest `origin/main` introduced a CSS-module purity error in `src/components/public-navigation.module.css` by using top-level `:global(.nav_wrap)` selectors. Moved that exported-nav hide rule into the existing inline critical style in `src/app/(site)/layout.tsx`, preserving the intended hidden Webflow nav while making `/__editor?path=%2Fcareers` compile again. Verification: local route returned `200`; `pnpm typecheck`, `pnpm lint`, and `git diff --check` passed.
- Editor image-source draft guard on 2026-06-18: visual-editor handoff showed bogus `imageSrc` content changes on non-image containers (`section`, `main`, and regular `div`) because the live DOM fallback grabbed the first descendant image. `src/app/(archive)/__editor/shell.tsx` now only creates/caches image-source draft changes when the selection capability says `canEditImage` (or the target is an actual `img`). The generated `public/editor-patches.css` from the `/careers` handoff keeps only CSS-supported changes: hero/main padding and hero-note paragraph line-height/tracking. Verification: `pnpm typecheck`, `pnpm lint`, `git diff --check`, and local `/__editor?path=%2Fcareers` returned `200`.
- Editor typography controls fix on 2026-06-18: text elements that contain inline markup, such as the `/careers` trust-row paragraph with nested `<strong>`, now show the Style tab Type controls without enabling unsafe raw text replacement. The bridge exposes `canStyleText` separately from strict `canEditText`; the sidebar uses `canStyleText` for font/size/weight/color visibility and keeps Content text editing gated by `canEditText`. Verification: `pnpm typecheck`, `pnpm lint`, `git diff --check`, and a local `/__editor?path=%2Fcareers` bridge smoke confirmed the selected trust paragraph shows Type/Font/Size while `Text content` remains hidden.
