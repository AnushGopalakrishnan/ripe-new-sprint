...
- Follow-up on 2026-07-22: careers mosaic captions that explicitly used Geist now use local Graphik Regular (`"Graphik"` at weight `400`) in `src/app/(site)/careers/careers-mosaic.module.css`; intentionally serif careers typography remains unchanged. Local computed-style verification found zero visible Geist elements on `/careers`, confirmed the affected caption at Graphik weight `400`, and found no horizontal overflow.
- Follow-up on 2026-07-22: the `/careers` hero headline now reads: `A brand design studio where everyone's success feels personal. We exist to support ambitious designers and founders building brands worth believing in.`
- Follow-up on 2026-07-22: the `/careers` hero headline uses the supplied typography settings at all breakpoints: Plantin MT Pro Light (`300`), `36px` font size, `48px` line height, `-0.16px` letter spacing, and left alignment.
