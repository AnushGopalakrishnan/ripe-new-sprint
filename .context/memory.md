...
- Follow-up on 2026-05-26: added a public-page shortcut for opening the visual editor. Any public page with `?editor` now redirects through `src/proxy.ts` to the existing editor route with the current pathname as the target, e.g. `/case-studies/zetachain?editor` -> `/__editor?path=/case-studies/zetachain`. The internal `/__editor?path=...` route remains the canonical editor implementation.
