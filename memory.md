...
- Case-study detail page refresh on 2026-05-15 (Polestar layout + comment overlays):
  - Reworked `src/app/(archive)/case-studies/[slug]/page.tsx` to render only the core Polestar-style media experience (navbar/footer intentionally removed per request) while preserving section geometry and spacing from the current Framer reference.
  - Added `src/app/(archive)/case-studies/[slug]/case-study-client.tsx` as a client renderer with:
    - mixed-media support for the first hero slot (`kind: "auto"`) and all major media sections (image/video/gif-compatible via URL-kind inference),
    - working carousel arrows + active dot states for the main gallery panel,
    - Figma-style inline comments over media: click-to-place pin, author/body composer, numbered pins, pin detail cards, and per-section localStorage persistence.
  - Updated `src/app/(site)/detail-page.module.css` with media class parity (`.formaHeroMedia`, `.formaSectionMedia`, `.formaCarouselPanelMedia`, `.formaBlackFeatureMedia`) and the comment overlay styles (`.formaComment*`) while preserving existing desktop/mobile layout behavior.
  - Validation: `pnpm typecheck` passed and `pnpm build` passed locally (Next.js reported the existing non-blocking NFT warning from `next.config.ts` import tracing).
- CMS-driven case-study detail conversion on 2026-05-15 (follow-up):
  - Replaced hardcoded Polestar detail payload with Sanity-backed case-study data fetch (`getCaseStudyBySlug`) in `src/app/(archive)/case-studies/[slug]/page.tsx`.
  - Added new `caseStudy` schema fields for detail layout content and per-media pinned comments:
    - `detailEyebrow`, `detailServices[]`, `detailIndustry`, `detailInformation[]`
    - commentable media groups: `detailHero`, `detailIntro`, `detailCarouselSlides[]`, `detailCarouselPoster`, `detailBlackFeature`, `detailWideFeature`, `detailCta`
    - `detailMoreProjects[]`
  - Extended GROQ case-study query projection to include those new fields plus nested pinned comment positions (`position.x/y`).
  - Extended `CaseStudy` TypeScript type with commentable media/comment position structures.
  - Frontend comments are now strictly CMS-rendered pins/cards (no localStorage or client-authored comment persistence); comment insertion remains Studio-side through schema/editor controls.
  - Validation: `pnpm typecheck` passed and `pnpm build` passed locally (same pre-existing NFT warning only).
- Case-study detail media source + 404 follow-up on 2026-05-15:
  - `mediaBlock` schema is now upload-capable (image/video + poster image) in addition to URL-based sources:
    - Added `image` (Sanity image), `video` (Sanity file), `posterImage` (Sanity image), with conditional visibility by kind.
    - Added media validation requiring at least one concrete source per kind (URL or uploaded asset).
  - GROQ projections now normalize media URLs using `coalesce(src, image.asset->url, video.asset->url)` and poster using `coalesce(poster, posterImage.asset->url)` across home/case-study/writing queries.
  - Case-study detail `[slug]` route now exports `dynamic = "force-dynamic"` to avoid stale/static not-found behavior when valid CMS entries exist.
  - Sanity check confirmed `case-study-10` exists as a published `caseStudy` (`_id: caseStudy.case-study-10`, title `Tabletop`) in the configured production dataset.
  - Validation: `pnpm typecheck` and `pnpm build` passed (same pre-existing Turbopack NFT warning).
- Route-group ownership update on 2026-05-15:
  - Moved the full case-studies route family from `src/app/(archive)/case-studies/*` into `src/app/(site)/case-studies/*`:
    - `/case-studies`
    - `/case-studies/[slug]`
    - `/case-studies/tags/[slug]`
    - detail client component co-located at `src/app/(site)/case-studies/[slug]/case-study-client.tsx`.
  - This keeps public case-study routes owned by the `(site)` app tree as requested.
  - Local validation after move: `pnpm build` passed and `pnpm typecheck` passed.
  - Note: stale `.next/dev/types/validator.ts` references to old archive paths were causing transient `tsc` errors; resolved by clearing the stale `.next/dev` generated artifacts and re-running checks.
- Legacy case-study slug alias on 2026-05-15:
  - Added redirect in `next.config.ts` from `/case-study-:id` to `/case-studies/case-study-:id` to support legacy direct slug URLs.
  - Confirmed via Sanity API that `case-study-19` exists and is published (`caseStudy.case-study-19`, title `Volvo`), so previous 404 was a route-shape mismatch, not missing CMS content.
  - Validation: `pnpm typecheck` and `pnpm build` passed.
